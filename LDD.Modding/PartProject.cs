...
namespace LDD.Modding
{
    ...
    public class PartProject
    {
        ...
        {
            return CreateFromLddPart(Core.LDDEnvironment.Current, partID);
        }

        public static PartProject CreateFromLddPart(Core.LDDEnvironment environment, int partID)
        {
            var lddPart = PartWrapper.LoadPart(environment, partID, true);
            return CreateFromLddPart(lddPart);
        }

        public static PartProject CreateFromLddPart(PartWrapper lddPart)
        {
        ...
        public Vector3d CalculateCenterOfMass()
        {
            ...
            Vector3d center = LDD.Common.Simple3D.Vector3d.Zero;
                ...
                foreach (var meshRef in meshRefs)
                {
                    ...
                    if (meshGeom != null)
                    {
                        foreach (var tri in meshGeom.Triangles)
                        {
                            ...
                            center += ((LDD.Common.Simple3D.Vector3d)(tri.V1.Position + tri.V2.Position + tri.V3.Position) / 4d) * currentVolume;
                        }
                    }
                }
                center /= totalVolume;
        ...
        public void CalculateBoneBoundingBoxes()
        {
            ...
            foreach (var bone in Bones.OrderBy(x => x.BoneID))
            {
                ...
                {
                    bone.Bounding = new BoundingBox(new Vector3d(-0.0001d), new Vector3d(0.0001d));
                    var physAttr = bone.PhysicsAttributes;
                    physAttr.CenterOfMass = Vector3d.Zero;
                    physAttr.Mass = 0;
                    physAttr.InertiaTensor = Matrix3d.Zero;
                }
            }
        ...
        public bool TryGetProperty<T>(string propertyName, out T value)
        {
            ...
            if (ProjectProperties.TryGetValue(propertyName, out string strValue))
                return StringUtils.TryParse(strValue, out value);
            return false;
        }
        ...
        public PartWrapper GenerateLddPart()
        {
            if (Bounding is null)
            ...
            var part = new PartWrapper(GeneratePrimitive());
